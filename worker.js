  ...
  async fetch(req, env) {
    ...
    if (url.pathname === '/tape') {
      ...
      if (!day) {
        ...
        return new Response(JSON.stringify({ days: st.tapeDays || {}, ttlDays: TAPE_TTL_DAYS, cols: ['id', 'bid_mc', 'ask_mc', 'vol24h'] }), { headers: cors });
      }
      ...
      return new Response(JSON.stringify({ day, snaps, cols: ['id', 'bid_mc', 'ask_mc', 'vol24h'] }), { headers: cors });
    }
    ...
    return new Response(JSON.stringify({
      ...
      tape: { days: s.tapeDays || {}, note: 'GET /tape?day=YYYY-MM-DD for the point-in-time book snapshots' },
      houseRisk: s.house ? (() => {
